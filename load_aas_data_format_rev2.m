function [output_data,cell_table]=load_aas_data_format_rev2(app,temp_data_label,excel_filename,tf_rescrap_excel)
...
data_filename2=strcat(temp_data_label,'_cell_table_azi_el_data.mat');
[var_exist_input2]=persistent_var_exist_with_corruption(app,data_filename2);

if tf_rescrap_excel==1
    var_exist_input=0;
end

if var_exist_input==2 && var_exist_input2==2
    retry_load=1;
    while(retry_load==1)
        try
            load(data_filename1,'output_data')
            load(data_filename2,'cell_table')
            pause(0.1)
            retry_load=0;
        catch
            retry_load=1;
    ...
    while(retry_save==1)
        try
            ...
            save(data_filename2,'cell_table')
            pause(0.1)
            retry_save=0;
        catch
            retry_save=1;
